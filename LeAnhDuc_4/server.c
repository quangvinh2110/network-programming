...
{ 
    ...
    int n; 
    int check = 0;
    int check_name = 0; // xem có phải yêu cầu nhập tên không
    int check_age = 0;   // xem có phải yêu cầu nhập tuổi không
    int userValid = 0;
    int ageValid = 0;
    int i;
    int send = 0;    // xem đã gửi thông báo đến client chưa
    ...
    for (;;) { 
        ...
        bzero(buff, MAX); 
        n = 0; 
        // nếu là yêu cầu bình thường thì nhập trả lại dữ liệu cho client
        
        while ((buff[n++] = getchar()) != '\n') 
            ; 
   
        // gửi cho người client
